...
class AdminAdmissionController extends Controller
{
    ...
    //     } catch (Exception $e) {
    //         return back()->with('error', 'Failed to load admissions: ' . $e->getMessage());
    //     }
    // }
    ...
    public function store(Request $request)
{
    ...
    $validator = Validator::make($request->all(), [
        ...
        'adhaar_card' => 'required|mimes:jpeg,png,jpg,pdf|max:2048',
    ], $messages);
    ...
    public function update(Request $request, $id)
{
    ...
    $validator = Validator::make($request->all(), [
     ...
     // 'email' => 'nullable|email|unique:admissions,email,' . $admission->id . '|max:255',
        ...
        'adhaar_card' => 'nullable|mimes:jpeg,png,jpg,pdf|max:2048',
    ...
    try {
        ...
        if ($request->hasFile('adhaar_card')) {
            $adhaar = $request->file('adhaar_card');
            $adhaarName = time() . '_' . $adhaar->getClientOriginalName();
            $adhaar->move(public_path('uploads/admissions/aadhar'), $adhaarName);
            $data['adhaar_card'] = 'uploads/admissions/aadhar/' . $adhaarName;
