    ...
    <div class="card-body">
        <ul class="nav nav-tabs" id="examTabs" role="tablist">
            <li class="nav-item" role="presentation">
                <a class="nav-link active" id="manage-tab" data-bs-toggle="tab" href="#manage" role="tab">
                    Manage Exam</a>
            </li>
            <li class="nav-item" role="presentation">
        ...
        <div class="tab-content mt-3">
            ...
            <div class="tab-pane fade show active" id="manage" role="tabpanel">
                ...
                <form method="GET" action="{{ route('exam.index') }}" class="row g-3 mb-3">
                    ...
                    <div class="col-md-3">
                        <select name="class_id" id="filter_class" class="form-select" required>
                    ...
                    <div class="col-md-3">
                        <select name="section_id" id="filter_section" class="form-select">
                    ...
                    <div class="col-md-3">
                        <select name="term" id="filter_term" class="form-select">
                ...
                <div class="alert alert-info">
                    Please select a Class & Section to manage exams.
                </div>
                @endif
            ...
            $.ajax({
                ...
                success: function(subjects) {
                    ...
                    $.each(subjects, function(i, subject) {
                        $('#subjects_id').append('<option value="' + subject.id +
...
<script>
    $(document).ready(function () {
    // On page load, populate section if class is already selected (for edit/filter)
    let selectedClassId = $('#filter_class').val();
    let selectedSectionId = '{{ request("section_id") }}';

    if (selectedClassId) {
        $.ajax({
            url: '/admin/get-sections/' + selectedClassId,
            type: 'GET',
            dataType: 'json',
            success: function (sections) {
                $('#filter_section').empty().append('<option value="">-- Choose Section --</option>');
                $.each(sections, function (i, section) {
                    let selected = section.id == selectedSectionId ? 'selected' : '';
                    $('#filter_section').append('<option value="' + section.id + '" ' + selected + '>' + section.name + '</option>');
                });
            },
            error: function () {
                $('#filter_section').empty().append('<option value="">-- Choose Section --</option>');
    ...
    // When class is changed, load its sections
    $('#filter_class').change(function () {
        let classId = $(this).val();
        $('#filter_section').empty().append('<option value="">Loading...</option>');
        ...
        if (classId) {
            $.ajax({
                ...
                success: function (sections) {
                    $('#filter_section').empty().append('<option value="">-- Choose Section --</option>');
                    $.each(sections, function (i, section) {
                        $('#filter_section').append('<option value="' + section.id + '">' + section.name + '</option>');
                    });
                },
                error: function () {
                    $('#filter_section').empty().append('<option value="">-- Choose Section --</option>');
